...
async function apiText(path: string, init?: RequestInit): Promise<{ ok: boolean; status: number; text: string }> {
  const res = await fetch(path, {
    credentials: "include",
    ...init,
  });
...
export default function App() {
  // Keep until auth + profile load is stable.
  ...
  // Optional: pre-login health check on load
  useEffect(() => {
    void fetchHealth();
  }, [fetchHealth]);
